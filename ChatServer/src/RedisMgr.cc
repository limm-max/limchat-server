...
bool RedisMgr::Incr(const std::string& key) {
    auto conn = _pool->getConnection();  
    if (conn == nullptr) {
        LOG_DEBUG<<"Redis连接获取失败";
        return false;
    ...
    redisReply* reply = (redisReply*)redisCommand(conn->get(), "INCR %s", key.c_str());
    ...
    if (reply->type != REDIS_REPLY_INTEGER) { freeReplyObject(reply); return false; }
    return true;
}


bool RedisMgr::Decr(const std::string& key) {
    auto conn = _pool->getConnection();  
    if (conn == nullptr) {
        LOG_DEBUG<<"Redis连接获取失败";
        return false;
    ...
    redisReply* reply = (redisReply*)redisCommand(conn->get(), "DECR %s", key.c_str());
    if (reply == nullptr) { return false; }
